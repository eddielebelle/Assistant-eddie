...
import torch
from transformers import AutoTokenizer, AutoModelForCausalLM, pipeline, AutoModelForSeq2SeqLM, T5Tokenizer, T5ForConditionalGeneration
...
from config import (
    ...
    WHISPER_MODEL_PATH, T5_MODEL_PATH, T5_TOKENIZER_PATH, SPACY_MODEL_PATH
)

class translator:
    def __init__(self, client_id='interface', mqtt_host=None, mqtt_port=None, mqtt_username=None, mqtt_password=None):
        ...
        self.audio_model = whisper.load_model(WHISPER_MODEL_PATH)
        # initialising LLM
        self.device = torch.device(0)
        self.tokenizer = T5Tokenizer.from_pretrained(T5_TOKENIZER_PATH)
        self.model = AutoModelForSeq2SeqLM.from_pretrained(T5_MODEL_PATH).to(self.device)
        ...
        self.nlpB = spacy.load(SPACY_MODEL_PATH)
        self.nlpS = spacy.load(SPACY_MODEL_PATH)
